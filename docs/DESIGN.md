...
**Identity, separate from the database's own bookkeeping.** A chunk's *identity* — the thing an evidence edge points at — is not the same as its FalkorDB node ID. Each chunk carries an `anchor_id`, derived once from its source file and heading path and never recomputed from content, plus a `content_hash` that changes only when the text actually does. Re-ingesting a doc resolves new chunks against this identity rather than deleting and recreating: unchanged content is untouched, edited content updates the same node in place, and a section moved to a new heading is recognized and carried forward — all without disturbing the node's incoming edges. A `(:Observation)-[:MOTIVATES]->(:Chunk)` edge, in particular, survives an ordinary edit to the doc it's attached to. Sections a re-ingest can't resolve are marked, not deleted, so evidence is never silently lost. `:CodeEntity` identity is treated more conservatively today — a `parent_name` key fixes the worst fragility (a line-shift edit elsewhere in the file no longer changes which node an entity is), but full rename detection via a signature hash is still open (fork issue #3).
...
- **A general-memory store, reachable and verified.** The `memory` graph (facts, observations) has schema, storage, and MCP tools — `add_observation`, `create_fact`, `link_evidence`, `search_observations`, `search_facts` — confirmed working end-to-end against a live FalkorDB + llama-server, including a real MCP `tools/call` round-trip. `:Protocol` is still schema-only (see issue #6). Agent-orchestrated multi-turn distillation (`propose_then_confirm` sessions) is still deferred. See [`ARCHITECTURE.md`](ARCHITECTURE.md#memory-facts-observations-protocols-planned) and `docs/IMPLEMENTATION.md` §1.
