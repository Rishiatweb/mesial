...
| **Aho–Corasick / better linker algorithm.** | Corrected from an earlier draft of this table: the linker is map-based (`nameToIDs`), not a nested `O(chunks × names)` scan — matching cost is ~O(total chunk content) and roughly independent of registered-name count. Benchmarked in `internal/doclinker/linker_bench_test.go`: a 1000× increase in names (100→100,000, fixed 200 chunks) moved runtime only ~1.5× (52ms→82ms); a 100× increase in chunks (200→20,000, fixed 10,000 names) moved it ~90× (54ms→4.9s) — i.e. linear in content, not quadratic in either dimension. Switching to Aho–Corasick isn't warranted by CPU cost at any scale tested. The real scaling risk lives elsewhere: `link()` issues one synchronous `ConnectEntities` Cypher round-trip per matched edge, so wall-clock cost on a real repo is dominated by DB round-trip count, not by the matching loop — see `docs/IMPLEMENTATION.md` §3, "Batch `DOCUMENTS` edge writes." |
