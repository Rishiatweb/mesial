...
| `internal/falkorstore` (`memorystore.go`) | `EnsureGraphMeta`, `AddObservation`, `AddFact`, `LinkEvidenceFor`, `LinkMotivates`, `SearchObservations`, `SearchFacts`, evidence/existence checks — 544 lines, full CRUD | **yes, as of the 5 memory tools below** — wired but **not build/runtime-verified** (see caveat below) | `memorystore_test.go` |
...
**Real caveat, not a formality:** this was implemented and manually cross-checked line-by-line against the actual `pipeline`/`falkorstore` function signatures, but it has **not been compiled or run**. No Go toolchain was available in the environment it was written in. `go build ./...`, and ideally a live smoke test against FalkorDB + llama-server + a real MCP client, are both still outstanding before this should be trusted or merged upstream.

## 2. Pathway: Implementation

Ordered by dependency, not just priority — later items build on earlier ones.

1. **~~Expose the memory layer via MCP~~ — implemented, needs build + smoke-test.** `add_observation`, `create_fact`, `link_evidence`, `search_observations`, `search_facts` are registered in `cmd/ingest/main.go`. This was LIFECYCLE.md's Tier 1 item "`add_observation` runtime capture" and the one Tier 1 item with nothing else blocking it — still true, it just moved from "not started" to "unverified" rather than "done." Next action here isn't more code, it's `go build ./...` + a real run against FalkorDB/llama-server.
