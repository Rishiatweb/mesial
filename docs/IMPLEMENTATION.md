...
Not covered by this pass: `analyze_repository`'s LSP-dependent path (needs `typescript-language-server`, not installed for this test) and the `motivates_chunk_id` folding branch specifically (no chunk nodes existed in the test graph — `evidence_for_fact_ids` folding was verified, the `MOTIVATES` sibling path uses the same tested `pipeline.LinkObservationMotivatesChunk` function so risk here is low, but it wasn't exercised through the MCP layer directly).
