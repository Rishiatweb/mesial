...
- **Memory graph: facts, observations, protocols.** A third graph kind alongside `code` and `notes`. Schema designed in [`docs/ARCHITECTURE.md#memory-facts-observations-protocols-planned`](docs/ARCHITECTURE.md#memory-facts-observations-protocols-planned): `:Observation` (sentence, embedded, episodic) ←`:EVIDENCE_FOR`→ `:Fact` (triplet, semantic, not embedded) plus `:Protocol` (procedural, schema TBD). Predicate kernel of seven inference-actionable predicates (`is_a`, `subtype_of`, `part_of`, `equivalent_to`, `incompatible_with`, `causes`, `requires`); arbitrary predicates allowed but inert. Reserved fact-axiom edges (`:ENTAILS`, `:INSTANCE_OF`, `:SUBTYPE_OF`, reified `:Rule` with `:PREMISE`/`:CONCLUDES`) for a future inference engine. `:GraphMeta {kind, strict}` singleton declares each graph's vocabulary. Implementation deferred.
- **Distillation MCP surface.** Agent-orchestrated multi-turn distillation: agent writes an observation → server returns similar facts (KNN over observations, traversed via `:EVIDENCE_FOR`) → agent confirms which to link and/or proposes a new triplet. Sessions ephemeral and TTL-bounded in the ingest server; tool design deferred until the data design is fully ratified.
- **Inference engine MCP tool.** Forward/backward chaining over `:Fact` triplets and the reserved axiom edges. Brain-region nickname TBD (candidates: frontopolar cortex / Brodmann area 10, the relational-integration region; or DLPFC for classical deduction).
